...
Global flags (`--json`, `--format`, `--quiet`, `--verbose`, `--no-color`, `--config`, `--path`, `--fail-on`, `--min-score`, `--fix`, `--offline`, `--version`, `--help`) apply to every command and live in `docs/PLAN.md` §2.2, together with the exit-code contract (0 clean · 1 a configured gate failed · 2 execution error · 3 config error). The two gates share exit code `1`: `--fail-on` fails the run when any finding ≥ its severity, `--min-score N` fails it when the Ship Score is below N (`check` only). They are **independent gates** — the run exits `1` if **either** trips and `0` only when **both** pass (OR-to-fail / AND-to-pass); execution (2) and config (3) errors still dominate. `--min-score` is global but inert (a documented no-op, not an error) on every command except `check`, the only one that computes a Ship Score.

`--format <human|json|agent>` selects the stdout renderer: `human` (default terminal render), `json` (the full findings envelope), or `agent` (a token-lean, LLM-shaped report — see below). `--json` is retained as a back-compat alias for `--format=json` and **conflicts with** `--format` (passing both is a clean CLI error). `--format` governs stdout only; `-o/--output` always writes the full JSON artifact regardless of `--format` (see below). SARIF and other formats remain the v0.5 `--format` roadmap slot — the flag reserves exactly these three values now.

**`--format=agent` (all findings commands: `check`/`real`/`audit`/`review`/`env`/`ship`):** a deterministic, ANSI-free, LLM-shaped report — a `GATE`/`SUMMARY` header, one dense line per finding carrying its `gdv1:` fingerprint, and a deduped `NEXT ACTIONS` checklist (full shape in `docs/SPEC-FINDINGS.md`). It carries the full machine list every time (no summary-by-default collapse), masks secrets like every renderer, and is measured smaller than `--json` on the reference corpus. `getdev check --format=agent --min-score 80` is the canonical agent-loop line.

`-o/--output <FILE>` (v0.1.3, findings commands: `check`/`real`/`audit`/`review`/`env`/`ship`): write the full JSON report (docs/SPEC-FINDINGS.md schema) to FILE while the terminal keeps a short summary — banner + top-3 (check) and the one-line tally, ending with `full report → FILE (N finding(s) · K KB)`. Combined with `--json`, stdout prints only the file path (script-friendly). The file is an explicitly requested artifact — no `--write` gate, overwrite allowed (eslint/trivy `-o` semantics). There is deliberately NO interactive output-format prompt: prompts after a computed report break CI/pipes and determinism. Instead the human terminal render is **summary-by-default** (B-06): a report longer than the threshold (>25 findings) collapses to the summary — score banner / severity tally — plus the top-3 worst findings and one reminder line, and does NOT print the full per-file findings list. Short reports (≤ threshold) render the full per-file list as usual, and the full list is always available on demand with `-v`/`--verbose`. This is **deterministic**, not TTY-dependent — the same output whether stdout is piped or a terminal — so CI logs, `grep`, and the corpus snapshots stay stable. Machine paths (`--json`, `-o`) are unaffected and always carry the complete report. `-o/--output` is unaffected by `--format` — it **always** writes the full JSON report artifact (D-11); `--format` governs stdout only, so `--format=agent -o file.json` writes JSON to the file and (like `--json -o`) prints the file path to stdout. Format selection beyond the three `--format` values (e.g. SARIF) remains the v0.5 `--format` roadmap item.
...
**Output:** score banner, findings grouped by severity, "top 3 things to fix first," fixable-count hint (`getdev env --write`, later `getdev fix`). `getdev check --json --fail-on high` is the canonical CI line; `getdev check --format=agent --min-score 80` is the canonical agent-loop line.

**Score gate (`--min-score N`):** `check --min-score N` fails the run (exit code `1`) when the Ship Score is below N. It shares exit code `1` with `--fail-on` and composes with it as independent gates — the run exits `1` if the findings gate **or** the score gate trips, and `0` only when both pass (see the exit-code contract in §2.2). `--min-score` is a global flag (its Ship-Score twin of `--fail-on`), effective here because `check` is the only command that computes a Ship Score; it is a documented no-op elsewhere. Under `--format=agent` the same gate verdict is rendered on the `GATE:` line, computed from the same source as the exit code so they can never disagree.

...
| `baseline` | object | **`check` only** (LOOP-03) — omitted (not `null`) unless a baseline was active this run (`--baseline`/`--update-baseline`/`--since`, or `[baseline].auto` with the file present). `{ mode, file?, snap?, suppressed }`: `mode` is `"persisted"` or `"since"`; `file` (string, the resolved baseline path) is present for `persisted` mode, `snap` (u32, the snapshot id) for `since` mode — the two are mutually exclusive and each omitted (not `null`) when not applicable; `suppressed` (usize) is the count of findings removed by the baseline this run. A run with no baseline active omits the field entirely, so `check --json` with no baseline flags is byte-unchanged (determinism). Mirrors the `-v` "N finding(s) suppressed by baseline" line (docs/SPEC-CONFIG.md §"Ordering, visibility, and staleness") so machine consumers see the same never-silent count. |
